 ...
 * @date 2019/12/6.
 */
class LinesLoadingView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    private fun initLoadingView(context: Context, attrs: AttributeSet) {
        val typeArray = context.obtainStyledAttributes(attrs, R.styleable.LinesLoadingView)
        val color = typeArray.getColor(
            R.styleable.LinesLoadingView_lines_color,
            ContextCompat.getColor(context, R.color.white)
        )
        mRate = typeArray.getInt(R.styleable.LinesLoadingView_rate,
            1000).toLong()
        mPaint = Paint()
        //第一个进度条画笔
