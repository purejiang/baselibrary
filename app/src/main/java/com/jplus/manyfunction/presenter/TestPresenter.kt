...
class TestPresenter : TestContract.Presenter {
    ...
    override fun checkToCameraOrPhoto(view: View, niceDialog: NiceAlertDialog) {
        ...
        when (view.id) {
            R.id.ntv_photo_dialog_camera -> {
                if (mIsCanOpen) {
                    mPhotoUtils?.openCamera("pic_" + System.currentTimeMillis() + ".jpg", true,
                    ...
                    mActivity?.let {
                        JPermissionsUtils.instance.showPermissionDialog(it, "权限提示", mutableSetOf(Manifest.permission.CAMERA))
                    }
                }
            }
    ...
    private fun uploadPic(url: String, path: String) {
        LogUtils.instance.d("ChoosePicture onSuccess，path:$path")
        ...
        OkhttpManager.uploadFile(url, body, object : Callback<ResponseBody> {
            override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                LogUtils.instance.d("请求失败：message:" + t.printStackTrace())
    ...
    override fun startPermissionTest() {
        mActivity?.let {
            JPermissionsUtils.instance.requestPermissions(it, null, object : JPermissionsUtils.PermissionListener {
                override fun ignoredCallback(permissions: MutableSet<String>) {
                ...
                override fun deniedCallback(permissions: MutableSet<String>) {
                    ...
                    if (Manifest.permission.SYSTEM_ALERT_WINDOW in permissions) {
                        JPermissionsUtils.instance.showWindowDialog(it, "权限提示", "为保证应用的正常使用，请开启允许显示在应用上层权限。")
                    }
                }
    ...
    override fun getAppInfos() {
        val appInfos =  AppUtils.instance.getAppsInfo(false, mActivity!!)
    ...
    override fun login(phone: String, password: String) {
        ...
        OkhttpManager.doPost("http://192.168.11.175:8000/login/", LoginRequest(phone, password), object : Callback<ResponseBody> {
            ...
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                if (response.isSuccessful) {
                    val str = response.body()?.string()
                    LogUtils.instance.d("登录成功：$str")
                } else {
                    LogUtils.instance.d("登录失败：code:" + response.code())
                }
            }
        })
    }
    ...
    override fun refreshLoadView() {
        mActivity?.startActivity(Intent(mActivity, RefreshActivity::class.java))
    }

    constructor(activity: Activity, view: TestContract.View) {
        mView = view
        mActivity = activity
        mView?.setPresenter(this)
        mPhotoUtils = PhotoUtils(activity, true)
        mDataSourceNice = NiceDownloadDataSource(activity.applicationContext!!)
    }
